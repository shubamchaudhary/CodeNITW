...
const MoneyTracking = () => {
  ...
  const handleSpendingChange = (category, value) => {
    setDailySpending(prev => ({ ...prev, [category]: value }));
  };
  ...
  const saveDailySpending = () => {
    Object.entries(dailySpending).forEach(([category, amount]) => {
      if (amount) {
        moneyTrackingService.addDailySpending(selectedDate, category, amount);
      }
    });
    toast.success("Daily spending saved!");
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
            {moneyTrackingService.CATEGORIES.filter(
              ...
              <div key={category.id} className="flex items-center space-x-2">
                ...
                <label className="text-sm dark:text-gray-300 flex-1">{category.name}</label>
                <input
                  type="number"
                  min="0"
                  placeholder="₹0"
                  value={dailySpending[category.id] || ""}
                  onChange={(e) => handleSpendingChange(category.id, e.target.value)}
                  className="w-24 px-2 py-1 border border-gray-300 dark:border-slate-600 rounded dark:bg-slate-700 dark:text-white text-right"
                />
              </div>
            ))}
          </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow-lg p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            {moneyTrackingService.CATEGORIES.filter(
              ...
              <div key={category.id} className="flex items-center space-x-2">
                <span className="text-2xl">{category.icon}</span>
                <label className="text-sm dark:text-gray-300 flex-1">{category.name}/month</label>
                <input
                  type="number"
                  min="0"
                  placeholder="₹0"
